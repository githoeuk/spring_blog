...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    // - 새로운 사용자를 Insert 하거나 , 기존 사용자 정보를 UPDATE합니다.
    // 2. 사용자 단건 조회 기능 : findByAll(Integer Id)
    // - PK(id)를 통해 특정 사용자를 조회하며 Optional<User>를 반환한다.
    // Optional이란? - 단건 조회 시 null(정보 없음)이 발생 시
    // NullPointException를 더 안정적으로 방지하기 위해 사용한다.
    ...
    // 사용자 정보 수정
    // [터티 체킹 이란?]
    // 1. 트랜잭션 내에서 조회된 객체의 상태를 변경하면
    // 트랜잭션이 끝나는 시점에 JPA가 변경된 내용을 자동으로 감시하여
    // DB에 UPDATE 쿼리를 날려준느 기능을 말한다.
